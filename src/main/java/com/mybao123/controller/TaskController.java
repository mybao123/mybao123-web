...
public class TaskController
{
	...
	public @ResponseBody
	String saveTask(Task task) throws Exception
	{   
		if (task == null)
		{
	...
	String loadListWithPage(long id) throws Exception
	{
		...
		if (user == null)
		{
			return JsonUtils.getJsonObject(0, ObjectStr, false,
					"用户登录过期，请退出重新登录");
		}
		try
		{
			List<Task> taskList = taskService.loadListWithPage(page,
					user.getId());
			if (taskList.size() > 0)
			{
			...
			return JsonUtils.getJsonObject(totalCount, ObjectStr, true,
					"获取记录列表成功");
		} catch (Exception ex)
		{
			return JsonUtils
					.getJsonObject(0, ObjectStr, false, ex.getMessage());
		}
	}

	@RequestMapping(value = "loadTaskList.do", produces = "application/json;charset=UTF-8")
	public @ResponseBody
	String loadList() throws Exception
	{
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder
				.getRequestAttributes()).getRequest();
		int totalCount = 0;
